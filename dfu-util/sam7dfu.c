		      ...
		      int xfer_size, AbstractFile* file)
{
	...
	char hash[] = {0xff, 0xff, 0xff, 0xff, 0xac, 0x05, 0x00, 0x01, 0x55, 0x46, 0x44, 0x10, 0x0, 0x0, 0x0, 0x0};
	...
	fflush(stdout);
	while (bytes_sent < totalSend /* - DFU_HDR */) {
		int hashes_todo;

		if(hashSent == 0) {
			ret = file->read(file, buf, xfer_size);
			if (ret < 0) {
				perror("read");
				goto out_close;
			}
			AppleCRC32Checksum(&crc, buf, ret);
		} else {
			ret = 0;
		}

		if(ret < xfer_size && hashSent == 0) {
		...
		if(ret < xfer_size) {
			int toSend = xfer_size - ret;
			if(toSend > (16 - hashSent))
				toSend = 16 - hashSent;

			memcpy(buf + ret, hash + hashSent, toSend);
			hashSent += toSend;
			ret += toSend;
		}
